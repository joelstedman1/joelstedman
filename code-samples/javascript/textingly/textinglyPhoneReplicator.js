...
$(function() {
		$("#btn_advanced").click(function() {
			if($("#advanced").attr("class")=='hide'){
				
				$("#advanced").removeClass("hide");
				$("#advancedHeader").removeClass("hide");
				$("#advancedHeader").html("Optional - Add TRIGGERS, special words that your customers can text in.");
				$("#command").val("COUPON");
				$("#trigger_response").val("Show this message to the cashier and you'll receive $5 off any item in our store.");
				$("#command_copy").html("COUPON");
				$("#trigger_response_copy").html("Show this message to the cashier and you'll receive $5 off any item in our store.");
			}
			else {
			
				$("#advanced").addClass("hide");
				$("#advancedHeader").addClass("hide");
				$("#command").val("");
				$("#trigger_response").val("");
				$("#command_copy").html("");
				$("#trigger_response_copy").html("");
			}
...
function transferToPhone() {
	...
	var senderId = 'command';
	var value = $("#"+senderId).val();
	...
	$("#"+destination).html(value);
		...
		$("#btn_cancel_new").click(cancelButtonWizard);
		$("#btn_cancel_block").click(cancelButtonWizardBlock);
		...
		$('textarea#message').keyup(countCharacters);
		$('textarea#response').keyup(countCharacters);
		//this attaches the counter to the textarea within the same parent div
		//$("textarea.attach_counter").focus(wizardAttachCounter);
		$('textarea#default_response').keyup(countCharacters);
		$('textarea#trigger_response').keyup(countCharacters);
		$('textarea#standard').keyup(countCharacters);
		...
		if(textingly_url1=="wizard") {
			...
			$("#trigger_response").keyup(wizardTransfer);
...
//			}
		...
		if((textingly_url1=="wizard")&&(textingly_url2=="one")) {
			...
			$("#trigger_response").one('focus', function() {
				$("#trigger_response").val('');
				$("#trigger_response_copy").html('');
			});
...
var creditWaitImage = function() {
	//console.log("now");
	$("#creditPurchase").trigger('click');
	$("#imageWait").removeClass("hide");
	$("#imageWaitUpgrade").removeClass("hide");
...
function confirmModalNew(yesFunction, vars, headerMsg, bodyMsg, buttonText) {
	if(vars.block=="block") {
		$("#btn_cancel_new").hide();
		$("#btn_cancel_block").show();
	}
	else {
		$("#btn_cancel_new").show();
		$("#btn_cancel_block").hide();
	}
	if($.browser.msie && $.browser.version=="7.0"){
		var confirmed = confirm(bodyMsg);
		//alert(confirmed);
		if (confirmed) {
			yesFunction(vars);
		}
		...
		$("#modal_message").empty();
		$("#confirm_message_header").html(headerMsg);
		$("#confirm_message_text").html(bodyMsg);
		$("#btn_ok_new").find('input').val(buttonText);
		$("#confirm_box_new").modal();
		
		$("#btn_ok_new").one("click", function() {
			yesFunction(vars);
			$.modal.close();
		});
	}
...
function confirm_upgrade(vars) {
	window.location = textingly_base+"pricing";
}
...
var changeKeyword = function() {
	...
	var keywordLink = "";
	if(plan>1) {
		keywordLink = "<br /><a href=\""+textingly_base+"keywords/lists\" class='link'>Purchase another Keyword.</a>";
	}
	else {
		keywordLink = "<br /><a href=\""+textingly_base+"pricing\" class='link'>Upgrade Plan for Multiple Keywords.</a>";
	}
	...
	if(keyword!="Select Your Keyword"){
		if(linked==1){
			var vars = {'block':'keyword'};
		    var headerMsg = "Confirm Keyword Switch";
		    	var bodyMsg = "This keyword is currently linked to an existing conversation. Unlinking will make your conversation inactive, and you will no longer be able to send or receive messages in that conversation.";
		    var buttonText = "Switch";
		    confirmModalNew(confirm_switch_keyword, vars, headerMsg, bodyMsg, buttonText);	
...
function confirm_switch_keyword (vars) {
	$.modal.close();
}

var cancelButtonWizard = function(e) {
	$("#keyword_select").val('1').siblings("span").html("Select Your Keyword");
	$.modal.close();
}
function confirm_upgrade (vars) {
	window.location = textingly_base+"pricing";
...
var cancelButtonWizardBlock = function(e) {
	window.location = textingly_base+"conversation/lists";
	$.modal.close();
}
...
var wizardSaveKeywordCommand = function() {
	...
	var auto_response = $("#trigger_response").val();
	if(auto=="") {
		auto_response = -1;
	}
	...
	if(newKeyword==1) {
		//var modalField = new Array("keyword_purchase","response", "command", "trigger_response");
		var modalField = new Array("keyword_purchase","response", "standard");
	}
	else {
		//var modalField = new Array("keyword_select","response", "command", "trigger_response");
		var modalField = new Array("keyword_select","response","standard");
	}
		//var modalHuman = new Array("Keyword","Greeting","Trigger", "Triggered Response");
	var modalHuman = new Array("Keyword","Greeting", "Standard Response");
		var modalType = "text";
			        ...
			        function(data){
				        ...
				        //if($("#keyword_purchase").val() != '') {
					        ...
					        return;
					        	...
					        	if($.browser.msie && $.browser.version=="7.0"){
					        		window.location = textingly_base + "wizard/two";
					        	}
					        	else {
							      	$("#contact_wizard_modal").modal({persist: true});
					        	}
							      	$.post (textingly_base+"index.php/ajax/addWizardInfo",//url
											{keywordCode: keywordCode, keyword: keyword, auto: auto, auto_response: auto_response, convo_response: convo_response, standard: standard, newKeyword: newKeyword},//params
											function(data){
												//alert("addwizardinfo");
...
var num_total = num_total;
	if ((textingly_url1=="campaign")&&(textingly_url2=="response")) {//conversation/response
		...
		$.post (""+textingly_base+"index.php/ajax/get_conversation_responses",
			 ...
			 function(data){
				initiatePaginationVariablesCampaign(pageTotal);
				}, "json");
	...
	var addEditButton = function(event) {
		...
		//$("#command").hide();
			...
			/* else {
				...
				$("#add_edit_text").html(addEdit + " Trigger");
				$(".default_response").hide();
				...
				$.post (textingly_base+"index.php/ajax/"+type+"_popup",//url
					...
					function(data){	
						...
						if(type=="command") {
							...
							$("#trigger_response").val(trigger_response);
						}
						else {
							if(textingly_url1=="wizard") {
...
function validateModal(field, human, type) {
				...
				var value = $.trim(value1);
				 ...
				 if(field[i]=="keyword_select") {
					 if(value==1){
						 ...
						 $('div#modal_error_'+field[i]).text(errorMsg);
						 errorMsgCount = errorMsgCount + 1;
						 alertModal('The Keyword field is required.')
					 }
				 }
	...
	var saveAddEditButton = function() {
		...
		if(type=="command") {
			...
			var modalHuman = new Array("Name","Greeting","Default Response");
			var modalType = "text";
	...
	//counts number of characters in message box
	var checkMessageCount = function() {
		...
		$("#"+type+"_count").html(limit);
		if(textingly_url1=="wizard") {
			$("span#"+type+"_count").show();
		}
		var count = $("#"+type).val().length;
		var remaining = limit-count;
		if(remaining<1) {
			$("#"+type+"_count").html('0');
...
var sendMessageButtonConfirmed = function() {
		...
		$("span.errorClear").html("");
		 ...
		 $.post(
	        ...
	        function(data){
	        	...
	        	if(data.space==1) {
	        		$("span#sorry").addClass("red");
        			$("span#sorry").html("Spaces or special characters aren't allowed.");
        		}
		...
		$.post(
	        ...
	        function(data){
	        	...
	        	if(data.space==1) {
			        ...
			        if(data.count > 0) {
			        	$("#sorry").html('<span class="red">Sorry, Trigger already exists for Conversation.</span>');
			        }
        	   }
	        }, 
	     "json");
	...
	//type in response to ajax function
		var title = $("#command").val();
		var response = $("#trigger_response").val();
		if(textingly_url1=="wizard") {
			var campaignId = $("#hdn_campaignId").val();
		}
		else {
		...
		$("#sorry").empty();
		//validate for empty fields
		var modalField = new Array("command","trigger_response");
			var modalHuman = new Array("Trigger","Triggered Response");
			var modalType = "text";
			var validate = validateModal(modalField, modalHuman, modalType);
